...
export default function Ticket({ navigation }) {
  ...
  useFocusEffect(React.useCallback(() => {
    const tmpArray = tempNotiData.map( (item) => {
      return {'vehicle':item.vehicle, 'time':item.time, 'device':item.device, 'checked':false};
    })
  ...
  const saveFile = async () => {
    const csv = jsonToCSV(resultData);
  ...
  return (
    <SafeAreaView style={{ backgroundColor: "white" }}>
      ...
      <Header back="true" screenName={t('tickets')} curNavigation={navigation}></Header>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', width: wp('97%'), marginTop: 21 }}>
        ...
        <Text style={[styles.listHeader,{width:wp('30%')}]}>{t('company')}</Text>
      </View>
