...
import { SafeAreaView, Text, TouchableOpacity, Image, View, Modal, ActivityIndicator,
  StatusBar, StyleSheet, Slider } from 'react-native'
import MapView, { Marker} from 'react-native-maps';
import { IconComponentProvider, Icon } from "@react-native-material/core";
import MaterialCommunityIcons from "@expo/vector-icons/MaterialCommunityIcons";
import { widthPercentageToDP as wp, heightPercentageToDP as hp } from 'react-native-responsive-screen';
import { useDispatch, useSelector } from 'react-redux';
import { saveGeofensePos } from "../actions/vehicles";
...
function Barriere({ navigation, route }) {
  ...
  const handlePressMap = async (coordinate) => {
    var tempPolyData = [...polygonData, coordinate];
    //https://maps.googleapis.com/maps/api/directions/json?origin=30.82688829457982,76.57527465373278&destination=30.414058942787786,76.72724660485983&mode=driving&sensor=false&units=metric&key=AIzaSyB_X2kKYs1hiQB2N4lCrhqQzLx9Te_0GG8
    //https://maps.googleapis.com/maps/api/directions/json?origin=30.965808903240454,76.5216351673007&destination=30.420093657133336,76.70834843069315&mode=BICYCLING&sensor=false&units=metric&key=AIzaSyB_X2kKYs1hiQB2N4lCrhqQzLx9Te_0GG8
    //https://console.cloud.google.com/google/maps-apis/discover/geolocation?project=ornate-keel-403613
    //https://developers.google.com/maps/documentation/directions/get-directions#TravelMode
    
    setPolygonData(tempPolyData)
  ...
  const handleAddGeo = () => {
    console.log("Handle Add Geofense", info.vehicle.teltonikas.deviceImei)
    ...
    dispatch(saveGeofensePos(token, info.vehicle.teltonikas.deviceImei, roadPointList, show3, show4, title, content, polygonData));
    navigation.goBack()
  ...
  return (
    <SafeAreaView style={{ marginTop: 20 }}>
      ...
      <MapView
        initialRegion={{
          latitude: info.vehicle.teltonikas.lat,
          longitude: info.vehicle.teltonikas.lng,
          latitudeDelta: 2,
          longitudeDelta: 1
        }}
        onPress={(e) => {          
      ...
      <View style={showStyle ? styles.view2 : styles.view21}>
        ...
        <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between'}}>
          ...
          <TouchableOpacity disabled={btnDisabledSave}
            onPress={handleAddGeo}
            style={styles.bigButton}>
            <Text style={{ color: '#FFFF', fontSize: 14, fontWeight: '500' }}>{t('save')}</Text>
          </TouchableOpacity>
        </View>
      </View>
    </SafeAreaView>
