...
import { LOG_OUT, SWITCH_MENU_VISIBLE, SWITCH_SIDEBAR_VISIBLE, TOGGLE_MAP_MODE } from "../../redux/store/types.js";
      ...
      {isMobile ? (
            ...
            <div className="d-flex flex-column position-absolute menu-mobile">
              ...
              <div className="d-flex flex-column main">
                <div className="d-flex flex-column px-4 py-3">
                  <p className="text-white" style={{fontSize: "2.25rem"}} >{user?.fname}</p>
                  <Button
                    className="ml-auto"
                    id="basic-button"
                  ...
                  >
                    <div >
                      <img src="./assets/common_user.png" alt="none" className="person-icon" />
                      <img src="./assets/polygon.png" alt="none" className="polygon-header mr-0" />
                    </div>
                  </Button>
                  <Menu
                    id="basic-menu"
                  ...
                  >
                    <MenuItem onClick={() => setAnchorEl(null)}><span style={{fontSize: "14px"}}>Profile</span></MenuItem>
                    <MenuItem onClick={() => setAnchorEl(null)}><span style={{fontSize: "14px"}}>My account</span></MenuItem>
                    <MenuItem onClick={handleLogout}><span style={{fontSize: "14px"}}>Logout</span></MenuItem>
                  </Menu>
                </div>
                <div className="horizontal-bar" />
                <div className="d-flex flex-column px-5 py-3">
                  {tabs.filter(tab => user.role === "Admin" ? true : !tab.onlyAdmin).map((tab, index) => (
                    <div key={index} className="tab my-4">
                      <NavLink
                        aria-current="page"
                        to={tab.to}
                        onClick={() => dispatch({type: SWITCH_MENU_VISIBLE, payload: false})}
                        className="px-3 d-flex justify-content-start py-2"
                        style={({ isActive }) => 
                          ({ color: isActive ? "#FFFFFF" : "#005EEC", backgroundColor: isActive ? "#1A2678" : "#FFFFFF", borderRadius: "30px"
                        })}
                      >
                ...
                <div className="d-flex mt-auto text-white ml-auto mr-5 mb-4 align-items-center" onClick={handleLogout}>
                  <p className="mb-0" style={{fontSize: "1.6rem", fontFamily: "rmedium"}}>Log Out</p>
        ...
        <div className="header">
          <div className="upper py-1">
            ...
            >
              <div >
                <img src={mapMode === 1 ? "./assets/gmap.svg" : "./assets/mapbox.svg"} alt="none" className={mapMode === 1 ? 'mx-4' : 'mapbox-icon px-2'} />
            ...
            >
              ...
              <MenuItem onClick={() => toggleMapMode(2)}>
                <img src="./assets/mapbox.svg" alt="none" className="mapbox-icon px-2" />
              </MenuItem>
            </Menu>
            <div className="notifications-icon">
              <img width={20} src="./assets/Bell.svg" alt="none" />
              <div className="position-absolute text-center text-white">{AlertCount}</div>
            </div>
            <p >{user?.fname}</p>
            <Button
              id="basic-button"
