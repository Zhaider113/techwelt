...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (mapMode === 2) {
      if (getData?.lng && getData?.lat) {
        ...
        if (devices?.length > 0) {
          const bounds = devices.reduce((acc, device) => {
            return [
  ...
  return (
    <div className="map-container position-relative d-flex justify-content-center w-100">
      {mapMode === 1 ? (
        ...
        >
          {devices?.map((device, index) => (
            <Marker key={index} latitude={device.vehicle.lat} longitude={device.vehicle.lng}>
      ...
      {isMobile && (
        <input className="field-input position-absolute" type="text" placeholder="Search vehicle..." value={searchText} onChange={(e) => setSearchText(e.target.value)} />
      )}
    </div>
  );
};
