...
const Signup = () => {
  ...
  useEffect(() => {
    const validatieUserName = debounce(async () => {
      if (userName) {
        var response = await getResponse("/auth/validateUserName", "post", {userName});
              ...
              <div className='input-container d-flex mb-4 position-relative'>
                ...
                {userName && (
                  <img 
                    ...
                    src={userNameValid ? './assets/valid.svg' : './assets/invalid.svg'}
                    style={{width: '1.8rem', right: '0.5rem', bottom: '0.7rem'}}
                    alt='none' 
                  />
                )}
              </div>
                  ...
                  <div className='suggestion position-absolute w-100 px-3 overflow-y-auto'>
                    {suggestions.map((cityData, index) => (
                      <p 
                        ...
                        key={index}
                        onClick={() => setCity(cityData.state + ", " + cityData.country)}
                      >
