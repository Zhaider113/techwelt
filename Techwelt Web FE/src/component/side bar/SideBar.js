...
const SideBar = (props) => {
  ...
  useEffect(() => {
    if (devices?.length > 0) {
      var result = devices.length > 0 && devices?.filter((item, index) => (!connectedTag || item?.vehicle?.isConnected === connectedTag) 
  ...
  const handleVehClick = (value) => {
    ...
    if (value && value !== selectedVeh) {
      setSelectedVehData(devices?.filter(d => d.vehicle.deviceImei === value)[0]);
    }
  ...
  return (
    ...
    {isMobile ? (
      <>
        {!showMenu && (
          <div
            className={`position-absolute d-flex justify-content-center align-items-center menu-icon w-100 ${showSidebar ? 'up-arrow' : 'down-arrow'}`}
            ...
            <div className="main-sidebar d-flex flex-column h-100 position-relative">
              <div className="filter d-flex flex-column my-5">
                ...
                <div className="sub1-div5 d-flex justify-content-end mt-4">
                  <p className="text-center text-white px-4 py-1 mb-2">Total Vehicles: &nbsp; {filteredVehicles?.length}</p>
                </div>
                ...
                {filteredVehicles?.map((item, index) => {
                  return (
                    <div
                      ...
                      className={`sub1-div6 d-flex my-2 py-3 ${item.vehicle.deviceImei === selectedVeh && 'selected'}`}
                    ...
                    >
                      <div className="left-sub1-div6 d-flex flex-column justify-content-between align-items-center px-2 py-4">
                        <img src={item.vehicle.title === "Car" ? "./assets/Car.svg" : "./assets/Bike.svg"} alt="none" className="my-2" />
                        <span className="text-white my-1 text-center w-100" style={{ backgroundColor: handleColor(item.vehicle.isConnected) }}>
                      ...
                      <div className="right-sub1-div6 d-flex flex-column px-2 justify-content-between">
                        ...
                        <div className="right-sub1-div6-sub1 d-flex align-items-center">
                          ...
                          <p className="mb-0">{formatDate(item?.vehicle?.teltonikas?.at(0)?.updatedAt)}</p>
                          <p className="mb-0">{formatTime(item?.vehicle?.teltonikas?.at(0)?.updatedAt)}</p>
                        </div>
                        <div className="right-sub1-div6-sub2 d-flex justify-content-evenly">
                          <img src="./assets/I.svg" id='mark' alt="none" style={{ cursor: "pointer" }} onClick={() => { handle1() }} />
                          <img src="./assets/Pointer.svg" id='location' alt="none" style={{ cursor: "pointer" }} onClick={() => { handleLocation(index) }} />
                          <img src="./assets/Location.svg" id='navigation' alt="none" style={{ cursor: "pointer" }} onClick={() => { handleNavigation(index) }} />
                        </div>
                      </div>
                    </div>
                  );
                    ...
                    <div className="car-portal-side-bar d-flex justify-content-center align-items-center">
                      <div className="sub1-car-portal-side-bar bg-white">
                        ...
                        <div className="subsub2-sub1-car-portal-side-bar d-flex align-items-center my-1 py-3 px-4">
                          ...
                          <p className="ml-3 mb-0">{selectedVehData?.vehicle?.vehicleName}</p>
                        </div>
                        <div className="subsub3-sub1-car-portal-side-bar my-1 py-2 px-4">
                          <div className="sub1-subsub3-sub1-car-portal-side-bar d-flex align-items-center">
                            ...
                            <p
                              className="mb-0 ml-4"
                              style={{ color: handleColor(selectedVehData?.vehicle?.isConnected) }}
                            >
                          ...
                          <div className="sub2-subsub3-sub1-car-portal-side-bar d-flex justify-content-center mt-2">
                            <p className="mb-0">{formatDate(selectedVehData?.vehicle?.teltonikas?.at(0)?.updatedAt)}</p>
                            <p className="mb-0 ml-2">{formatTime(selectedVehData?.vehicle?.teltonikas?.at(0)?.updatedAt)}</p>
                          </div>
                        </div>
                        <div className="subsub3-sub1-car-portal-side-bar my-1 py-2 px-4">
                          <div className="sub1-subsub3-sub1-car-portal-side-bar d-flex align-items-center">
                            <img src="./assets/gps.png" alt="none" />
                            <p className="mb-0 ml-4">{selectedVehData?.vehicle?.gpsFixed === 1 ? "Not Fixed" : "GPS Fixed"}</p>
                          </div>
                          <div className="sub2-subsub3-sub1-car-portal-side-bar d-flex justify-content-center mt-2">
                            <p className="mb-0">{formatDate(selectedVehData?.vehicle?.teltonikas?.at(0)?.updatedAt)}</p>
                            <p className="mb-0 ml-2">{formatTime(selectedVehData?.vehicle?.teltonikas?.at(0)?.updatedAt)}</p>
                        ...
                        <div className="subsub6-sub1-car-portal-side-bar d-flex justify-content-center align-items-center">
                          <p className="mb-0">IMEI: {selectedVehData?.vehicle?.deviceImei}</p>
                        </div>
                      </div>
                      ...
                      <div className="sub2-car-portal-side-bar h-100 d-flex flex-column justify-content-between align-items-center px-0 py-5">
                          ...
                          <div className="sub1-subsub1-sub2-car-portal-side-bar d-flex col-3 flex-column align-items-center">
                            ...
                            <p className="mt-5 mb-0 text-white">
                              {selectedVehData?.vehicle?.teltonikas?.length !== 0 ?
                                selectedVehData?.vehicle?.teltonikas[selectedVehData?.vehicle?.teltonikas?.length - 1]?.speed
                              : ""}KM/HR
                            </p>
                          </div>
                          <div className="sub1-subsub1-sub2-car-portal-side-bar d-flex col-3 flex-column align-items-center">
                            ...
                            <p className="mt-5 mb-0 text-white text-center">
                              Engine{" "}
                              {selectedVehData?.vehicle?.engineStatus}
                            </p>
                          </div>
                          <div className="sub1-subsub1-sub2-car-portal-side-bar d-flex col-3 flex-column align-items-center">
                            ...
                            <p className="mt-5 mb-0 text-white">
                              {selectedVehData?.vehicle?.teltonikas.length !== 0 ?
                                selectedVehData?.vehicle?.teltonikas[selectedVehData?.vehicle?.teltonikas?.length - 1]?.fuel
                              : ""}L
                            </p>
                          </div>
                          <div className="sub1-subsub1-sub2-car-portal-side-bar d-flex col-3 flex-column align-items-center">
                            <img src="./assets/Pic4.svg" alt="none" />
                            <p className="mt-5 mb-0 text-white">{selectedVehData?.vehicle?.temperature} C</p>
                          </div>
                        </div>
                        ...
                        <div className="subsub1-sub2-car-portal-side-bar w-100 d-flex">
                          <div className="sub1-subsub1-sub2-car-portal-side-bar d-flex col-3 flex-column align-items-center">
                            ...
                            <img className="mt-5 mb-0" style={{width: "1.5rem", height: "1.5rem"}} src="./assets/Group.svg" alt="none" onClick={() => setSimState(true)} />
                          </div>
                          <div className="sub1-subsub1-sub2-car-portal-side-bar d-flex col-3 flex-column align-items-center">
                        ...
                        <div className="subsub1-sub2-car-portal-side-bar w-100 d-flex">
                          <div className="sub1-subsub1-sub2-car-portal-side-bar d-flex col-3 flex-column align-items-center">
                            ...
                            <label className="car-switch mt-5">
                              <input
                                type="checkbox"
                                onClick={(e) => {
                                  handleEngine(e, selectedVehData?.vehicle.deviceImei);
                                }}
                              />
                          ...
                          <div
                            ...
                            onClick={() => {
                              navigate("/CommandsSettings", { state: { Name: selectedVehData?.vehicle.vehicleName, device: selectedVehData?.vehicle.deviceType + ' ' + selectedVehData?.vehicle.deviceModel, deviceImei: selectedVehData?.vehicle?.deviceImei } });
                            }}
                          >
                            <img src="./assets/Pic10.svg" alt="none" />
                            <p className="text-center text-white mt-5 mb-0">Commands<br/>Setting</p>
                          </div>
                          <div
                            className="sub1-subsub1-sub2-car-portal-side-bar d-flex col-3 flex-column align-items-center"
                            onClick={() => handlUpload()}
                          >
                            ...
                            <p className="mb-0 mt-5" style={{ color: uploadText === "Upload Config" ? "white" : "#F89823" }}>{uploadText}</p>
                          </div>
                        </div>
                      </div>
                    </div> : ''
      ...
      <div className="d-flex flex-column sidebar">
        <div className="logo py-1 d-flex align-items-end ml-4">
          <img src="./assets/logo.png" alt="none" onClick={() => navigate("/Dashboard")} />
            ...
            {filteredVehicles?.map((item, index) => {
              return (
                <div
                  key={item.vehicle.deviceImei}
                  id={"veh-" + item.vehicle.deviceImei}
                  className={`sub1-div6 d-flex my-2 py-2 ${item.vehicle.deviceImei === selectedVeh && 'selected'}`}
                  onClick={() => handleVehClick(item.vehicle.deviceImei)}
                >
